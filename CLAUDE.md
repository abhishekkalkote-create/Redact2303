# RedactProof — Build Guide for Claude Code
...
- **Workers:** Python on SQS + ECS Fargate; PyMuPDF (fitz) + pikepdf for PDF work; Textract (scans) with Tesseract fallback; Microsoft Presidio + custom regex for deterministic detection; Amazon Bedrock (Claude) for contextual detection behind `app/llm/provider.py` abstraction.
